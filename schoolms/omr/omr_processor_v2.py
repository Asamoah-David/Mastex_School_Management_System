...
def process_omr_image_v2(
    ...
    """Delegates to production pipeline in `omr.pipeline` (blank subtraction + CC marks)."""
    from .pipeline import process_omr_image_v2 as _run

    return _run(image_path, template_config, thresholds, generate_debug_image)
...
def enhanced_quality_check(image_path: str, template_config: dict | None = None) -> dict:
    """Delegates to `omr.pipeline` (Laplacian, brightness, coverage, tilt, shadows)."""
    from .pipeline import enhanced_quality_check as _q

    return _q(image_path, template_config)
